...
void Mesh::accept(Renderer* renderer) {
	renderer->visit(this);
}
...
glm::vec3 Mesh::getHightestValues(GameObject* g) {
	debug(" WHEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH ");
	// compute an AABB from the vertices of this mesh
	glm::vec3 maxValues = glm::vec3(0, 0, 0);
	for (vector<glm::vec3>::iterator iter = vertices.begin(); iter != vertices.end(); iter++) {
		glm::vec3 v = (*(iter));
	...
	vector<GameObject*>* children = g->getChildren();
	for (vector<GameObject*>::iterator i = children->begin(); i != children->end(); ++i) {
		glm::vec3 v = ((GameObject*) *i)->getMesh()->getHightestValues(((GameObject*) *i));
		if (v.x > maxValues.x) {
			maxValues.x = v.x;
		}
		if (v.y > maxValues.y) {
